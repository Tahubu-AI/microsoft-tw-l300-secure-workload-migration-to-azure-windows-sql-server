...
6. On the **Custom deployment** page, enter values for the required ARM template parameters.
   ...
   - **Sqlmi V Cores**: Select `8`.
   - **Repository Branch**: Leave the default value of `main`.
   - **Repository Name**: Accept the default value.
   - **Repository Owner**: Select `microsoft`.
   - Select **Review + create** to continue to the next tab.
...
7. On the **Review + create** tab, review the terms and conditions and select **Create**.
    ...
    {: .highlight }
    The custom deployment can take around one hour to complete. Monitor progress via the Azure portal to ensure there are no problems.
