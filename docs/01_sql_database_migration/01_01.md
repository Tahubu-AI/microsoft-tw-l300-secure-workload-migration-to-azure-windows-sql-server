...
2. Log into the simulated SQL Server VM from Hyper-V Manager.
3. Install and configure Azure Arc on the VM to register it with Azure.
4. Install the Azure extension for SQL Server to connect the SQL Server to Arc.
...
5. In **Hyper-V Manager**, select **WINSERVER** from the list of resources on the left, then right-click **OnPremSqlVM** in the Virtual Machines list and select **Connect...** in the context menu.

    ![The Hyper-V Manager is shown with the on-premises SQL VM selected in the VM list and Connect highlighted in the context menu.](../../Hands-on%20lab/images/hyper-v-manager-onprem-sql-vm-connect.png)

6. In the connection window, select **Connect** and log in to the VM using the following credentials:
...
16. At the PowerShell prompt, change directories to the location where you saved the downloaded file.
    ...
    {: .note}
    > You can simplify the commands by coping the downloaded file to `C:\scripts` or a similar short path.
...
17. Run the following command:
    ...
    {: .important}
    > If you receive the error, 'Cannot install Azure Connected Machine agent on an Azure Virtual Machine. Azure Connected Machine Agent is designed for use outside Azure,' you are likely on the Hyper-V host machine and not the OnPremSQLVM. Verify you are on the right VM and try again.
...
20. When the script completes, verify you see the message that "Azure Extension for SQL Server was successfully installed."

21. Return to the Azure Arc **SQL servers** page in the Azure portal and verify you see the `OnPremSQLVM` server and databases.
