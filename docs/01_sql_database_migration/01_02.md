...
30. To confirm, navigate your lab resource group in the Azure portal and select the `tailspin<uniqueid>-sqlmi` SQL managed instance resource.

    ![The SQL managed instance resource in highlighted in the resource group.](../../Hands-on%20lab/images/azure-portal-sql-managed-instance.png)

31. On the SQL managed instance blade, expand **Data management** in the left menu, select **Database**, and verify the `ToyStore` database appears in the list of databases.

    ![The ToyStore database is highlighted in the Databases blade for the SQL managed instance resource.](../../Hands-on%20lab/images/azure-sql-mi-databases.png)

32. Congratulations! You have successfully establish the link between Tailspin's on-premises `ToyStore` database and SQL MI. In the next task, you will migrate the `ToyStore` database.
