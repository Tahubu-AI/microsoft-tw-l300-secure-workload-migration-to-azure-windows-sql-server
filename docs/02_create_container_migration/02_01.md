...
* This will take several minutes and you will need to restart the Hyper-V VM after Docker has been installed

![Docker Install](../../Hands-on%20lab/images/02-docker-install.png)

13. After the restart, verify Docker was installed correctly.  You might need to update `wsl` after reboot by issuing the command `wsl -update` from a powershell instance to allow Docker Desktop to start on the Hyper-V VM.

```powershell
    docker info
```

![Docker Verification](../../Hands-on%20lab/images/02-docker-verify.png)
...
15. Add the executable to a folder already on the path or place it in a new folder and add that folder to your environment path on the Hyper-V VM

16. Verify that kubectl is installed correctly on your Hyper-V VM:
...
17. Install Visual Studio Code on the Hyper-V VM if it is not already installed. You can download it from [here](https://code.visualstudio.com/docs/setup/windows)

18. Clone the repo for `Container Migration Solution Accelerator` from GitHub on your Hyper-V VM.  The repo is available at [https://github.com/microsoft/Container-Migration-Solution-Accelerator.git](https://github.com/microsoft/Container-Migration-Solution-Accelerator.git).
